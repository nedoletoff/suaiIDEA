...
static void
ideaInvertKey(word16
  const * EK, word16 DK[IDEAKEYLEN]) {
  int i;
  uint16 t1, t2, t3;
...
void
idea_key(idea_ctx * c, unsigned char * key) {
  ideaExpandKey(key, c -> ek);
  ideaInvertKey(c -> ek, c -> dk);
}
void
idea_enc(idea_ctx * c, unsigned char * data, int blocks) {
  int i;
...
void
idea_dec(idea_ctx * c, unsigned char * data, int blocks) {
  int i;
  unsigned char * d = data;
  for (i = 0; i < blocks; i++) {
    ideaCipher(d, d, c -> dk);
